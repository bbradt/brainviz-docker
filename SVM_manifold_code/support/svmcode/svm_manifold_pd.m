...
for k=1:kn1
    ...
    model_precomputed = svmtrain(label(train_list), [(1:train_sum)', kernel_(train_list,p_list)], sprintf('-q -t 4 -c %f',c));
    [dec_label(:,k), accuracy_P, dec_values(:,k)] = svmpredict(label(kni_list), [(1:test_sum)', kernel_(kni_list,p_list)], model_precomputed, '-q');
end
