...
class GICACommandInputSpec(GIFTCommandInputSpec):
    ...
    mask = traits.Str(
        mandatory=False,
        desc="Enter file names using full path of the mask. If you wish to use default mask leave it empty",
    )
    ...
    df = traits.Int(
        mandatory=False, desc="number of reduction steps used in the first pca step"
    )
    which_analysis = traits.Int(
        mandatory=False,
        desc="Options are 1, 2, and 3. 1 - standard group ica, 2 - ICASSO and 3 - MST. ",
    )
    group_ica_type = traits.Str(
        mandatory=False, desc="1 - Spatial ica, 2 - Temporal ica."
    )
    perfType = traits.Int(
        mandatory=False,
        desc="Options are 1, 2, and 3. 1 - maximize performance, 2 - less memory usage  and 3 - user specified settings. ",
    )
    dummy_scans = traits.Int(mandatory=False, desc="enter dummy scans")
    prefix = traits.Str(
        mandatory=False, desc="Enter prefix to be appended with the output files"
    )
    group_pca_type = traits.Str(
        mandatory=False, desc="options are subject specific and grand mean"
    )
    backReconType = traits.Int(
        mandatory=False,
        desc="options are 1 - regular, 2 - spatial-temporal regression, 3 - gica3, 4 - gica, 5 - gig-ica",
    )
    preproc_type = traits.Int(
        mandatory=False,
        desc="options are 1 - remove mean per timepoint, 2 - remove mean per voxel, 3 - intensity normalization, 4 - variance normalization",
    )
    ...
    scaleType = traits.Int(
        mandatory=False,
        desc="options are 0 - No scaling, 1 - percent signal change, 2 - Z-scores",
    )
    algoType = traits.Int(
        mandatory=False, desc="options are 1 - Infomax, 2 - Fast ica , ..."
    )
    refFiles = InputMultiPath(
        File(exists=True),
        argstr="-i %s",
        mandatory=False,
        position=0,
    ...
    display_results = traits.Int(
        mandatory=False, desc="0 - No display, 1 - HTML report, 2 - PDF"
    )
    network_summary_opts = traits.Dict(
        mandatory=False, desc="dictionary containing network summary options"
    )
...
class GICACommand(GIFTCommand):
    ...
    def _make_matlab_command(self):
        ...
        if isdefined(self.inputs.numWorkers):
            ...
            commandstr.append(
                "parallel_info.num_workers = %d;" % (self.inputs.numWorkers)
            )
        ...
        if isdefined(self.inputs.network_summary_opts):
            ...
            try:
                ...
                network_threshold = 1
            commandstr.append(
                "network_summary_opts.threshold = %s;\n" % str(network_threshold)
            )
            ...
            try:
                ...
                commandstr.append(
                    "network_summary_opts.structFile = '%s';\n" % (structFile)
                )
            except:
                pass
            ...
            commandstr.append(
                "network_summary_opts.convert_to_z = '%s';\n" % (convert_to_z)
            )
...
class evalGIFTCommandInputSpec(GIFTCommandInputSpec):
    ...
    args = traits.Str(mandatory=False, desc="Arguments for command")
...
class DFNCCommandInputSpec(GIFTCommandInputSpec):
    ...
    comp_network_names = traits.Dict(
        mandatory=True, desc="dictionary containing network names and network values"
    )
    TR = traits.Float(mandatory=True, desc="Enter experimental TR in seconds")
    dfnc_params = traits.Dict(
        mandatory=False, desc="dictionary containing dfnc parameters"
    )
    postprocess = traits.Dict(
        mandatory=False, desc="dictionary containing post-processing parameters"
    )
...
class MancovanCommandInputSpec(GIFTCommandInputSpec):
    ...
    features = traits.List(
        mandatory=False,
        desc="Enter features like spatial maps, timecourses spectra, fnc correlations",
    )
    covariates = traits.Dict(
        mandatory=False,
        desc="covariates. Each covariate must contain a list like category of covariate, file name, transformation (optional)",
    )
        ...
        if isdefined(self.inputs.univariate_tests):
            ...
            if univ_keys[0].lower() == "ttest2" or univ_keys[0].lower() == "ttest":
                ...
                if len(ttest_datasets) > 2:
                    raise Exception(
                        "Dataset error: Nested lists cannot be greater than length of 2"
                    )
